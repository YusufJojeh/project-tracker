...
$pendingStmt = $pdo->prepare( "
    SELECT
        ...
        s.created_at AS submission_date,  -- Use created_at instead of updated_at
        p.project_id,
        p.title AS project_title,
        u.username AS student_name  -- This fetches the student's name from the users table
    FROM stages s
    JOIN projects p ON s.project_id = p.project_id
    JOIN users u ON p.student_id = u.user_id  -- Ensure that the student ID from the projects table is joined with the users table
    WHERE p.supervisor_id = ? AND s.status = 'submitted'
    ORDER BY s.created_at DESC  -- Use created_at instead of updated_at
    LIMIT 5
" );
...
$projectsStmt = $pdo->prepare( "
    SELECT
        ...
        p.title,
        p.status,
        st.full_name AS student_name,  -- Display the student's name from the students table
    ...
    FROM projects p
    LEFT JOIN stages s ON p.project_id = s.project_id
    LEFT JOIN students st ON p.student_id = st.student_id  -- Join with the students table to fetch the student name
    WHERE p.supervisor_id = ?
