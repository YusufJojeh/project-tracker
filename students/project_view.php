...
// Accept either ?project_id= or ?id=
$projectId = isset($_GET['project_id']) ? (int)$_GET['project_id'] : (int)($_GET['id'] ?? 0);
$studentId = $_SESSION['user_id'];

// Load the project, ensure it belongs to this student
$stmt = $pdo->prepare("
    SELECT p.*, stu.username AS student_name, sup.username AS supervisor_name
    FROM projects p
...
// Load project stages
$stagesStmt = $pdo->prepare("SELECT * FROM stages WHERE project_id = ? ORDER BY due_date ASC");
$stagesStmt->execute([$projectId]);
$stages = $stagesStmt->fetchAll(PDO::FETCH_ASSOC);

// Prepare attachments query
$attachStmt = $pdo->prepare("SELECT * FROM attachments WHERE stage_id = ? ORDER BY created_at ASC");
          ...
          <div class="card-body">
            <div class="row mb-2">
              ...
              <div class="col-md-6">
                ...
                <br>
                <strong>Supervisor:</strong> <?php echo htmlspecialchars($project['supervisor_name']); ?><br>
                <strong>Created:</strong> <?php echo date('Y-m-d H:i', strtotime($project['created_at'])); ?><br>
                <?php if ($project['updated_at']): ?>
                <strong>Updated:</strong> <?php echo date('Y-m-d H:i', strtotime($project['updated_at'])); ?><br>
                <?php endif; ?>
              </div>
            </div>
          </div>
